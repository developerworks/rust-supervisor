...
```rust
impl RoleDefaultPolicy {
    /// Returns the default policy pack for the given task role.
    pub fn for_role(role: TaskRole) -> Self {
        match role {
            TaskRole::Service => Self::SERVICE_DEFAULT,
            TaskRole::Worker => Self::WORKER_DEFAULT,
            TaskRole::Job => Self::JOB_DEFAULT,
            TaskRole::Sidecar => Self::SIDECAR_DEFAULT,
            TaskRole::Supervisor => Self::SUPERVISOR_DEFAULT,
        }
    }
}
```
...
impl EffectivePolicy {
    ...
    pub fn merge(role: Option<TaskRole>, user_overrides: Option<UserPolicyOverrides>) -> Self {
...
- 若 **`sidecar_config`** 存在但 **`task_role`** 不是 **`Sidecar`**, 发出警告 (配置不一致)
