...
fn checked_files(repository_root: &Path) -> Vec<PathBuf> {
    // Specs remain at the workspace root after the workspace split.
    ...
    let documents = [
        ...
        workspace_root.join("specs/003-supervisor-dashboard"),
    ];
    let sibling_root = workspace_root.parent().expect("sibling root should exist");
