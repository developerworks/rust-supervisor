...
- UI(用户界面) browser test(浏览器测试) 使用 `wss://` relay(中继) URL(统一资源定位符) 和本地 TLS(传输层安全协议) WebSocket(网络套接字协议) 协议测试服务验证交互.
- 真实 mTLS(双向传输层安全协议认证) 证书链需要由部署环境提供, 本次测试覆盖配置, 身份派生和拒绝路径, 不覆盖真实证书签发.
- relay(中继) 到 target IPC(目标进程进程间通信) 的安全顺序由真实 Unix domain socket IPC(Unix 域套接字进程间通信) 测试目标覆盖, 没有启动真实三进程端到端联调.
