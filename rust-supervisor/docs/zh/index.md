...
工程文档不得描述 legacy wrapper(遗留包装), migration layer(迁移层) 或 deprecated facade(废弃门面). 关闭文档必须使用 Shutdown Without Orphaned Tasks(关闭后不留下孤儿任务). 配置文档必须使用 rust-config-tree v0.2.0 作为集中 YAML(数据序列化格式)配置边界.
