...
## Decision(决策): 事件发布采用 at-least-once(至少一次) 语义, 仅在 watchdog(看门狗) 自身崩溃时允许丢失

**Rationale(理由)**: 操作者需要在控制循环异常退出后可靠收到通知, 因此 normal path(正常路径) 下必须保证事件写入 journal(事件日志), metrics(指标) 和 audit(审计). at-least-once(至少一次) 语义让消费者可以安全依赖事件来进行告警和复盘. 仅当 watchdog(看门狗) 自身在发布事件前崩溃时, 该事件的丢失是可以接受的——因为此时整个运行时控制面已经不可用, 健康状态中保留的失败原因成为最后的可信记录.

**Alternatives considered(备选方案)**:

- exactly-once(恰好一次) 语义: 被拒绝, 因为在分布式或进程内异步管道中保证恰好一次需要持久化存储和去重逻辑, 超出当前规格范围.
- at-most-once(至多一次) 语义: 被拒绝, 因为它不能满足操作者对故障信号可靠到达的期望.
