...
| **`BudgetDenied`**            | `What::BudgetDenied` in `src/event/payload.rs`            | 新增 006-5, 预算拒绝事件             |
| **`GenerationFenced`**        | `What::GenerationFenced` in `src/event/payload.rs`        | 新增 006-5, 代次隔离事件             |
| **`HealthCheckPassed`**       | `What::HealthCheckPassed` in `src/event/payload.rs`       | 新增 006-5, 健康检查通过             |
| **`HealthCheckFailed`**       | `What::HealthCheckFailed` in `src/event/payload.rs`       | 新增 006-5, 健康检查失败             |
| **`Paused`**                  | `What::Paused` in `src/event/payload.rs`                  | 新增 006-5, 暂停监督                 |
| **`Resumed`**                 | `What::Resumed` in `src/event/payload.rs`                 | 新增 006-5, 恢复监督                 |
| **`Quarantined`**             | `What::Quarantined` in `src/event/payload.rs`             | 新增 006-5, 隔离                     |
| **`BackpressureAlert`**       | `What::BackpressureAlert` in `src/event/payload.rs`       | 新增 006-5, 背压告警                 |
| **`BackpressureDegradation`** | `What::BackpressureDegradation` in `src/event/payload.rs` | 新增 006-5, 背压降级                 |
| **`AuditRecorded`**           | `What::AuditRecorded` in `src/event/payload.rs`           | 新增 006-5, 审计记录                 |
| **`BackpressureConfig`**      | `BackpressureConfig` in `src/spec/supervisor.rs`          | 新增 006-5, 背压配置                 |
| **`BackpressureStrategy`**    | `BackpressureStrategy` in `src/spec/supervisor.rs`        | 新增 006-5, 背压策略                 |
| **`CorrelationHandle`**       | `CorrelationHandle` in `src/event/correlation.rs`         | 新增 006-5, 关联句柄                 |
| **`CorrelationQueryError`**   | `CorrelationQueryError` in `src/event/correlation.rs`     | 新增 006-5, 关联查询错误             |
| **`FiniteF64`**               | `FiniteF64` in `src/event/payload.rs`                     | 新增 006-5, 有限浮点包装             |
