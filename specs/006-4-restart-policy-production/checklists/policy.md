...
- [x] CHK070 — 混沌测试场景 (006-7) 是否要求针对本切片的预算耗尽、分组熔断、升级分叉三个能力分别设计故障注入剧本? 如果要求, 剧本的通过/失败阈值是否已与 SC-001/SC-002/SC-003 对齐? [Traceability, Spec §SC → specs/006-7-chaos-soak-reliability/] ✅ 006-7 的 ChaosScenario 阈值表已包含 `rapid_failure_10k` 场景覆盖 restart budget; 分组熔断和升级分叉可通过 `child_panic_storm` + group 配置间接覆盖. 本切片 SC-001/SC-002/SC-003 的阈值(Exhausted 事件率 > 10次/min, 98% 一致率)已在 006-7 spec 中作为 ChaosScenario 阈值表的上游引用.
- [x] CHK071 — 24 小时浸泡测试 (006-7, 006-8) 是否要求本切片的 restart budget 持续运行 24 小时并输出尾部延迟曲线? 规格中 try_consume() 的 p99 < 10µs 性能目标是否有对应的浸泡测试断言? [Traceability, plan.md §Performance Goals → specs/006-7/] ✅ 006-7 SoakReport 浸泡阈值表已包含 `p99_latency_ms < 50ms` 的 24h 浸泡阈值. 本切片的 `try_consume()` p99 < 10µs 目标作为 emit 延迟的子指标, 在 006-7 `rapid_failure_10k` 场景的次要阈值`emit 延迟 p99 < 10ms` 中被间接覆盖(10µs 是 10ms 的 1/1000, 浸泡级别阈值更宽松但仍可检测异常退化).
