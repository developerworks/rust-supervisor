...
│   ├── restart-budget-api.md
│   └── group-isolation-api.md
├── checklists/          # 检查清单
│   └── policy.md
...
> **基于实际代码库的软件工程多维度复杂度分析。数据来源: `src/policy/*.rs`, `src/observe/*.rs`, `src/runtime/*.rs`, `src/event/payload.rs`, `src/spec/*.rs`。**

### 认知复杂度 (Cognitive Complexity)

| 文件                          | LOC   | 最大嵌套深度 | 关键复杂函数                                                                                                      | 评估                                                                    |
| ----------------------------- | ----- | ------------ | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `src/runtime/control_loop.rs` | ~1500 | 6 层         | `handle_child_exit()` 多路径退出分支; `execute_shutdown()` 三阶段编排                                             | ⭐⭐⭐⭐⭐ 极高: 异步 + 多条件 + 状态机组合, 修改需跨 6 层嵌套理解      |
| `src/runtime/pipeline.rs`     | ~1000 | 5 层         | `execute_pipeline()` 六阶段协调; `stage_decide_action()` 多层决策树; `build_policy_aware_what()` 四优先级事件选择 | ⭐⭐⭐⭐ 高: 管道跨阶段状态累积, budget/meltdown/backoff 三条决策链交织 |
| `src/policy/meltdown.rs`      | ~620  | 4 层         | `evaluate_outcome_for_scopes()` 三级范围检查; `prune()` 嵌套循环多队列修剪                                        | ⭐⭐⭐ 中高: 三个独立作用域的熔断判定叠加                               |

### 数据复杂度 (Data Complexity)

| 文件                          | 关键数据结构                                                                                                   | 复杂度说明                                                                                                 |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `src/policy/meltdown.rs`      | `HashMap<ChildId, VecDeque<Instant>>` + `HashMap<String, VecDeque<Instant>>` + `HashMap<String, GroupCounter>` | 三个独立 HashMap 维护 child 级、group 级、supervisor 级三维失败跟踪, 需确保更新时三表原子一致              |
| `src/runtime/pipeline.rs`     | `PipelineContext` — 15 个字段的大聚合体                                                                        | 横跨 6 个管线的上下文, 承载 exit/classification/budget/meltdown/backoff/decision 全部中间状态              |
| `src/runtime/control_loop.rs` | `RuntimeControlState` — 14 个字段                                                                              | 管理 control_loop 全部可变状态: shutdown/shutdown_pipeline/slots/registry/observability/budget/fairness 等 |
| `src/event/payload.rs`        | `What` 枚举 — 30+ 变体, 多数带关联数据                                                                         | 本切片新增 3 个变体(BudgetExhausted/GroupFuseTriggered/EscalationBifurcated), 单变体最多携带 6 个字段      |
...
| 配置结构体       | 字段数 | 关键验证逻辑                                                                                                     |
| ---------------- | ------ | ---------------------------------------------------------------------------------------------------------------- |
| `SupervisorSpec` | 20+    | `validate()` 链式调用含 `validate_group_strategies()`、`validate_group_membership()` 等跨字段约束, 嵌套深度 4 层 |
| `ChildSpec`      | 16     | 含 `Option<Arc<dyn TaskFactory>>` trait 对象, 验证需检查 factory 存在性 + 标签 + 策略一致性                      |

### 状态管理复杂度

| 状态机                                | 状态数                                                  | 关键风险                                                                                            |
| ------------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `RuntimeControlState` shutdown 三阶段 | `Idle -> Draining -> Aborting -> Reconciling -> Done`   | 异步编排中有竞态可能: `pending_restart` 释放时序与 `CancellationToken` 传播顺序不同步时导致孤儿任务 |
| `MeltdownTracker` 分组熔断            | `Normal -> ChildQuarantined -> GroupIsolated -> Normal` | `reset_after` 倒计时 + 零星故障不重置规则要求精确定时器管理                                         |
| `RestartBudgetTracker` 令牌桶         | `Idle -> Granted -> Exhausted -> Idle`                  | `try_consume()` 内 refill/evict/check 三步在 `&mut self` 下原子完成, 无锁安全但禁止外部并发访问     |

### 测试复杂度

| 维度             | 现状                                                             | 评估                                                                        |
| ---------------- | ---------------------------------------------------------------- | --------------------------------------------------------------------------- |
| 单元测试覆盖     | 19 个测试文件, `policy_budget_waveform_test.rs` 覆盖波形限流边界 | ✅ 充分                                                                     |
| 分组隔离对照实验 | `policy_group_isolation_test.rs` 含 24h 滑动窗口模拟             | ✅ 覆盖                                                                     |
| loom 并发测试    | 未发现明确的 loom 测试                                           | ⚠️ 缺口: `control_loop.rs` 的异步竞态风险无 loom 覆盖, 依赖夜间 CI 独立验证 |
| 性能断言         | `try_consume()` p99<10µs, `evaluate_budget` p99<100µs            | ⚠️ 阈值已定义但未嵌入 CI 持续性能门禁                                       |

### 安全复杂度

| 项目        | 结果                                                                                    |
| ----------- | --------------------------------------------------------------------------------------- |
| `unsafe` 块 | 全部相关文件 0 个 `unsafe` — 100% 安全 Rust ✅                                          |
| 锁竞争      | budget/fairness/meltdown 均为单线程控制循环内使用 `&mut self`, 无 `Mutex`/`RwLock` 开销 |
