# Quickstart(快速开始): 005-2 Task Role Defaults(任务角色默认值)
...
1. `src/policy/task_role_defaults.rs`
   - `TaskRole(任务角色)` 定义 5 类角色: `service(常驻服务)`, `worker(工作任务)`, `job(一次性作业)`, `sidecar(辅助任务)`, `supervisor(嵌套监督器)`.
...
2. `src/spec/child.rs`
   - `ChildSpec(子任务规格)` 包含 `task_role` 和 `sidecar_config`.
...
6. `src/event/payload.rs`
   - `SupervisorEvent(监督事件)` 包含 `task_role`, `used_fallback_default`, `effective_policy_source`.

7. `tests/task_role_defaults_integration.rs`
...
- id: orphan-sidecar
  task_role: sidecar
  command: ["fluentd"]
  # 错误: sidecar task_role requires sidecar_config
...
```bash
cargo fmt
cargo test --test task_role_defaults_integration
cargo test
...
```bash
cargo test --test task_role_defaults_integration job_success_exit_does_not_request_restart
cargo test --test task_role_defaults_integration service_success_exit_allows_restart
cargo test --test task_role_defaults_integration worker_failure_default_uses_bounded_retry
cargo test --test task_role_defaults_integration worker_default_restart_limit_feeds_budget_evaluation
cargo test --test task_role_defaults_integration sidecar_failure_default_restarts_only_sidecar_scope
cargo test --test task_role_defaults_integration supervisor_role_default_uses_outer_unit_restart_budget
```
...
```bash
cargo test --test task_role_defaults_integration sidecar_missing_config_is_rejected
cargo test --test task_role_defaults_integration sidecar_unknown_primary_is_rejected
cargo test --test task_role_defaults_integration sidecar_chain_is_rejected
cargo test --test task_role_defaults_integration job_permanent_restart_conflict_is_reported
```
...
```bash
cargo test --test task_role_defaults_integration emitted_pipeline_event_carries_policy_attribution
cargo test --test task_role_defaults_integration supervisor_event_fields_exist_for_policy_source
```

预期结果:

- `SupervisorEvent(监督事件)` 包含 `task_role`.
...
### Q4: 当前版本是否支持运行中修改 task_role(任务角色)?

不支持. `task_role(任务角色)` 属于监督单元启动前配置. 运行中需要改变角色时, 调用方必须重建该监督单元, 不能把角色热更新套到已经运行的子任务上.

## 5. 参考资料

- 功能规格: `specs/005-2-task-role-defaults/spec.md`
- 实现计划: `specs/005-2-task-role-defaults/plan.md`
- 研究结论: `specs/005-2-task-role-defaults/research.md`
- 数据模型: `specs/005-2-task-role-defaults/data-model.md`
- 接口契约: `specs/005-2-task-role-defaults/contracts/task-role-defaults.md`
