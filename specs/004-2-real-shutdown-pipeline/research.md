...
**Decision(决定)**: 第一次 `ShutdownTree(关闭监督树)` 创建并执行关闭流水线, 该次响应中 `idempotent(幂等)` 必须为 `false`. 同一次关闭仍在进行中时再次请求, 控制循环必须返回同一流水线的当前进度, 且 `idempotent` 必须为 `true`. 关闭已完成后的再次请求必须返回已缓存的最终报告, 且 `idempotent` 必须为 `true`.
