...
本功能把 `ShutdownTree(关闭监督树)` 从阶段推进改成真实的 runtime shutdown pipeline(运行时关闭流水线). 控制循环必须向所有运行中的 child task(子任务) 发送 `CancellationToken(取消令牌)`, 再按 `shutdown_order(关闭顺序)` 等待任务完成, 超时后对滞留任务执行 `abort(强制中止)`, 最后对 registry(注册表), runtime handles(运行时句柄), journal(日志), metrics(指标) 和 dashboard-visible diagnostics(仪表盘可见诊断) 做对账. 核心 runtime(运行时) 不直接拥有 dashboard IPC socket(仪表盘进程间通信套接字) 时, 对账报告必须把 socket(套接字) 标记为 NotOwned(非运行时拥有). `ShutdownCoordinator(关闭协调器)` 继续只负责阶段和幂等状态, `src/shutdown/report.rs` 拥有公开报告类型, `runtime(运行时)` 模块拥有任务句柄, 取消令牌和关闭执行.
...
研究结论写入 `specs/004-2-real-shutdown-pipeline/research.md`. 结论是复用 `tokio-util::sync::CancellationToken` 和 `tokio::task::AbortHandle`, 不新增依赖. 公开报告类型必须属于 `src/shutdown/report.rs`, 运行时只负责生成这些报告. 控制循环必须保存 active attempt(活动尝试) 的 token(令牌), abort handle(强制中止句柄) 和完成接收端. 当前 `ChildRunner::run_once` 内部的二级 `tokio::spawn` 会让外层任务中止后无法保证真实 child future(子任务 future) 被中止, 所以实现必须调整 runner(运行器) 的句柄边界.
