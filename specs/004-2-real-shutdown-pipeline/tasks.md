...
- [ ] T009 在 `src/control/command.rs` 中确认 `CommandResult::Shutdown(关闭命令结果)` 继续返回扩展后的 `ShutdownResult(关闭结果)`, 并且不新增控制命令变体.
...
- [ ] T017 [US1] 在 `src/runtime/control_loop.rs` 中确保关闭期间自动 restart policy(重启策略) 不会重新启动被取消的 child(子任务).
...
- [ ] T018 [US2] 在 `src/tests/supervisor_real_shutdown_pipeline_test.rs` 中添加关闭等待顺序必须符合 `shutdown_order(关闭顺序)` 的失败优先测试.
- [ ] T019 [US2] 在 `src/tests/supervisor_real_shutdown_pipeline_test.rs` 中添加优雅完成 child(子任务) 进入 `ChildShutdownStatus::Graceful` 的失败优先测试.
- [ ] T020 [P] [US2] 在 `src/tests/supervisor_control_test.rs` 中添加 `ShutdownTree(关闭监督树)` 返回完成报告的控制命令回归测试.

### Implementation for User Story 2(用户故事二的实现)

- [ ] T021 [US2] 在 `src/runtime/shutdown_pipeline.rs` 中使用 `src/tree/order.rs` 的 `shutdown_order(关闭顺序)` 生成稳定等待顺序.
- [ ] T022 [US2] 在 `src/runtime/shutdown_pipeline.rs` 中实现 `GracefulDrain(优雅排空)` 阶段, 并按 `ShutdownPolicy.graceful_timeout` 限制总等待预算.
- [ ] T023 [US2] 在 `src/runtime/shutdown_pipeline.rs` 中把正常返回的 `ChildRunReport(子任务运行报告)` 转换为 `ChildShutdownOutcome(子任务关闭结果)`.
- [ ] T024 [US2] 在 `src/runtime/control_loop.rs` 中把关闭等待期间到达的 `ChildAttemptMessage(子任务尝试消息)` 归并到正在执行的关闭流水线.
- [ ] T025 [P] [US2] 在 `src/observe/metrics.rs` 中记录 `shutdown_duration_seconds(关闭耗时秒数)` 的完整流水线耗时.
...
- [ ] T026 [US3] 在 `src/tests/supervisor_real_shutdown_pipeline_test.rs` 中添加忽略取消的 child task(子任务) 超时后被强制中止的失败优先测试.
- [ ] T027 [US3] 在 `src/tests/supervisor_real_shutdown_pipeline_test.rs` 中添加重复 `ShutdownTree(关闭监督树)` 返回缓存报告和 `idempotent(幂等)` 标记的失败优先测试.
- [ ] T028 [US3] 在 `src/tests/supervisor_real_shutdown_pipeline_test.rs` 中添加迟到 child exit(子任务退出) 被归并或标记为 `LateReport(迟到报告)` 的失败优先测试.
- [ ] T029 [P] [US3] 在 `src/tests/observability_smoke_test.rs` 中添加强制中止, 迟到报告和对账摘要的观测测试.
- [ ] T030 [P] [US3] 在 `tests/dashboard_protocol_shape_test.rs` 中添加 dashboard protocol(仪表盘协议) 未改变 `ShutdownTree(关闭监督树)` 请求形状的回归断言.
...
- [ ] T038 [P] 在 `manual/zh/observability.md` 中补充真实关闭流水线的 event(事件), metrics(指标) 和 audit(审计) 观测说明.
- [ ] T039 [P] 在 `README.zh.md` 中把 `ShutdownTree(关闭监督树)` 描述更新为真实取消, 等待, 强制中止和对账.
- [ ] T040 在 `src/tests/supervisor_docs_sync_test.rs` 中更新文档同步断言, 确保文档提到真实关闭流水线.
- [ ] T041 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo fmt --check`.
- [ ] T042 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo test --test supervisor_real_shutdown_pipeline_test`.
- [ ] T043 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo test --test supervisor_control_test --test supervisor_shutdown_test --test observability_smoke_test`.
- [ ] T044 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo test --test dashboard_protocol_shape_test`.
- [ ] T045 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo test --test naming_contract_test source_code_uses_approved_state_names`.
- [ ] T046 按 `specs/004-2-real-shutdown-pipeline/quickstart.md` 运行 `cargo test -- --skip checked_artifacts_avoid_forbidden_state_terms`.
