...
- **SC-002**: 本条是对 **FR-001** 的可测重写. 对所有 `RestartChild`(重启子任务) 相关集成测试, 必须在 100%(百分百) 场景中观察到先收敛当前活动 `attempt`(尝试), 再起新 `generation`(代次) 与新 `attempt`(尝试); `CurrentState`(当前状态) 与命令结构化结果不得呈现与该顺序矛盾的事实.
