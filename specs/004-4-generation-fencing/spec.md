# Feature Specification(功能规格): 代际隔离重启

**Feature Branch(功能分支)**: `004-runtime-semantics`
**Created(创建日期)**: 2026-05-14
**Status(状态)**: Draft(草稿)
**Input(输入)**: 用户描述: "当前 RestartChild(重启子任务) 直接生成一个新 attempt(尝试), 没有先停止旧 attempt(尝试). 如果旧任务还没有退出, 可能出现同一个 child id(子任务标识) 有多个运行实例. 正确做法是引入 generation fencing(代际隔离): 每个 child slot(子任务槽位) 同一时间最多一个 active attempt(活动尝试), 重启前先发取消, 等待或中止旧任务, 新任务启动时检查 generation(代数), 旧任务迟到上报时必须被丢弃或记为 stale report(过期报告)."
...
2. **Given(假设)** 旧尝试在关闭预算内退出, **When(当)** 系统启动新尝试, **Then(则)** 新尝试必须使用新的 generation(代数).
...
**Independent Test(独立测试)**: 同时触发手动 RestartChild(重启子任务) 和自动重启决策, 验证槽位不会启动第二个活动尝试.

**Acceptance Scenarios(验收场景)**:

1. **Given(假设)** 旧尝试尚未结束, **When(当)** 第二个重启请求到达, **Then(则)** 系统必须拒绝, 合并或排队该请求, 但不得启动第二个活动尝试.
2. **Given(假设)** 自动重启和手动重启同时发生, **When(当)** 槽位做出决策, **Then(则)** 系统必须保留一个明确的活动尝试.

---

### User Story 3(用户故事三) - 处理迟到的旧代报告 (Priority(优先级): P3)

操作者需要系统在旧 generation(代数) 的任务迟到上报时保护新状态, 并给出可审计诊断.

**Why this priority(为什么是这个优先级)**: 旧任务迟到上报如果覆盖新状态, 会让操作者看到错误的生命周期结论.

**Independent Test(独立测试)**: 启动新 generation(代数) 后模拟旧 generation(代数) 的退出报告, 验证系统把它丢弃或记录为 stale report(过期报告), 且不会覆盖新尝试状态.

**Acceptance Scenarios(验收场景)**:

1. **Given(假设)** 新 generation(代数) 已经运行, **When(当)** 旧 generation(代数) 迟到上报退出, **Then(则)** 系统不得用旧报告覆盖新槽位状态.
2. **Given(假设)** 旧报告被判定为 stale report(过期报告), **When(当)** 操作者查看事件或诊断, **Then(则)** 系统必须显示旧代数, 当前代数和处理结果.

### Edge Cases(边界情况)

- 旧尝试拒绝响应取消时, 重启流程必须走强制中止路径.
- 新尝试启动失败时, 槽位必须保留旧尝试的最终结果和新尝试失败原因.
...
- **FR-001**: RestartChild(重启子任务) 必须先停止当前 active attempt(活动尝试), 再启动新的 generation(代数) 和 attempt(尝试).
- **FR-002**: 每个 child slot(子任务槽位) 在任意时刻最多只能拥有一个 active attempt(活动尝试), 手动重启和自动重启都必须遵守该约束.
- **FR-003**: 旧 generation(代数) 的迟到报告必须被丢弃或记录为 stale report(过期报告), 并且不得覆盖当前 generation(代数) 的槽位状态.

### Key Entities(关键实体)

- **GenerationFence(代际隔离)**: 表示用 generation(代数) 保护槽位状态的规则.
- **ActiveAttempt(活动尝试)**: 表示 child slot(子任务槽位) 当前唯一允许运行的任务尝试.
- **StaleReport(过期报告)**: 表示旧 generation(代数) 在新 generation(代数) 生效后到达的报告.
...
- **Module ownership(模块所有权)**: runtime(运行时) 模块负责代际隔离和报告接收, policy(策略) 模块只产出重启决策.
- **Compatibility exports(兼容导出)**: None(无)
- **Diagnostics(诊断)**: 必须记录重启请求, 旧尝试停止结果, 新代数启动, 重启冲突和 stale report(过期报告).
...
- **SC-003**: 旧 generation(代数) 的迟到报告在 100% 的测试场景中不会覆盖当前 generation(代数) 状态.
- **SC-004**: 重启冲突场景中, 100% 的命令结果都包含冲突或排队处理结论.

## Assumptions(假设)

- 本规格依赖 `004-3-child-slot-control` 提供的 child slot(子任务槽位) 和真实活动尝试状态.
- 手动重启和自动重启必须使用同一个代际隔离规则.
