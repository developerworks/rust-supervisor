...
///
/// Each role binds to a distinct `RoleDefaultPolicyPack` that defines
...
## 8. RoleDefaultPolicyPack(角色默认策略包) 结构
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct RoleDefaultPolicyPack {
...
pub struct EffectivePolicy {
    ...
    pub policy_pack: RoleDefaultPolicyPack,
...
└─────────────────────────────┘
    ↓
Lookup RoleDefaultPolicyPack (查找角色默认策略包)
    ↓
...
- **005-2** 仅提供 **`RoleDefaultPolicyPack`** 作为 **`005-1`** 流水线中 **`decide action`** 阶段的输入增强
