...
/// Verifies that the declared dependency version is rust-config-tree v0.2.2.
#[test]
fn cargo_uses_rust_config_tree_v022() {
    let cargo = fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("Cargo.toml"))
        .expect("read Cargo.toml");

    assert!(cargo.contains("rust-config-tree = \"0.2.2\""));
}
