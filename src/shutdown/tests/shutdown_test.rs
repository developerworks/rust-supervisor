...
// ---------------------------------------------------------------------------
// ShutdownPolicy unit tests (G1, G6)
// ---------------------------------------------------------------------------
...
/// Verifies that `ShutdownPolicy::default()` uses the recommended constants.
#[test]
fn shutdown_policy_default_uses_recommended_constants() {
    let policy = ShutdownPolicy::default();
    assert_eq!(policy.graceful_timeout, Duration::from_secs(5));
...
/// Verifies that `ShutdownPolicy` serializes and deserializes all fields
/// correctly (G7).
#[test]
fn shutdown_policy_roundtrip_serialization() {
    let policy = ShutdownPolicy::new(
    ...
    let deserialized: ShutdownPolicy = serde_json::from_str(&json).expect("deserialize");
