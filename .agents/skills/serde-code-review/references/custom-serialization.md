...
JSON numbers are IEEE 754 doubles. Values outside binary floating precision ranges get silently truncated.

```rust
// BAD for monetary values - binary floating numbers lose precision
#[derive(Serialize, Deserialize)]
struct Invoice {
    amount: String,  // BAD: monetary value lacks a checked numeric type
}
...
3. Are monetary/precision values using `Decimal`, not binary floating primitives?
